"""Beer Notes — Markdown-file-based storage engine.
...
class StorageConflictError(OSError):
    """Raised when a note changed outside Beer Notes after it was loaded."""
        ...
        if path.exists():
            ...
            if (
                ...
                raise StorageConflictError(
                    f"Note changed outside Beer Notes: {note.title}"
                )
        elif note._storage_revision:
            raise StorageConflictError(
                f"Note was removed outside Beer Notes: {note.title}"
            )
        if touch:
            note.touch()
        try:
